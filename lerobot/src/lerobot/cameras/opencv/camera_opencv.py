...
class OpenCVCamera(Camera):
    ...
    def _configure_capture_settings(self) -> None:
        ...
        # Apply manual exposure if configured; otherwise keep camera's auto-exposure.
        if self.exposure is not None:
            self.videocapture.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1)   # 1 = manual (V4L2)
            self.videocapture.set(cv2.CAP_PROP_EXPOSURE, float(self.exposure))
            actual_exp = int(self.videocapture.get(cv2.CAP_PROP_EXPOSURE))
            logger.info(f"{self} exposure set to {actual_exp} (requested {self.exposure})")
